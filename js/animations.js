...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // gsap.registerPlugin(ScrollTrigger); // ScrollTrigger is already included via CDN

  console.log("animations.js loaded and GSAP is available:", typeof gsap !== 'undefined');
  ...
  if (typeof gsap !== 'undefined' && !prefersReducedMotion) {
    ...
    gsap.set(["body", "#home", "#home-sub", "#benefiting", "#about", "#footer", ".home-headline"], { opacity: 1 });
  }
  ...
  if (typeof gsap !== 'undefined' && !prefersReducedMotion) {
    ...
    if (statsTons) {
      gsap.from(statsTons, {
        ...
        onUpdate: function() {
          statsTons.textContent = Math.ceil(gsap.getProperty(this.targets()[0], "textContent"));
        },
        onComplete: function() { // Ensure final value is exact
            statsTons.textContent = "478";
        }
      });
       // Set the final value directly for GSAP to animate *to* it.
      gsap.to(statsTons, { textContent: 478, duration: 2, ease: "power1.inOut", snap: { textContent: 1 }, scrollTrigger: { trigger: statsTons, start: "top 85%", toggleActions: "play none none none", once: true } });
    }

    if (statsColleges) {
      gsap.from(statsColleges, {
        textContent: 0,
        duration: 2,
        ease: "power1.inOut",
        snap: { textContent: 1 },
        ...
        onUpdate: function() {
          statsColleges.textContent = Math.ceil(gsap.getProperty(this.targets()[0], "textContent"));
        },
        onComplete: function() { // Ensure final value is exact
            statsColleges.textContent = "40";
        }
      });
      // Set the final value directly for GSAP to animate *to* it.
      gsap.to(statsColleges, { textContent: 40, duration: 2, ease: "power1.inOut", snap: { textContent: 1 }, scrollTrigger: { trigger: statsColleges, start: "top 85%", toggleActions: "play none none none", once: true } });
    }
  }
  ...
  if (typeof gsap !== 'undefined' && !prefersReducedMotion) {
    const pastEvents = document.querySelectorAll(".events-past .calendar-event");
  ...
  if (typeof gsap !== 'undefined' && !prefersReducedMotion) {
    const ctaButtons = document.querySelectorAll(".btn-donate, .btn-default.navbar-btn"); // Target specific CTA classes
  ...
  if (typeof gsap !== 'undefined' && !prefersReducedMotion) {
    const dropdownMenuItems = document.querySelectorAll(".navbar-default .navbar-nav .dropdown-menu > li");
    const dropdownToggle = document.querySelector(".navbar-default .navbar-nav .dropdown-toggle");
    ...
    const mainNavLinks = document.querySelectorAll("#navbar .navbar-nav > li:not(.dropdown):not(.social):not(.navbar-donate) > a");
    mainNavLinks.forEach(link => {
      link.addEventListener("mouseenter", () => {
        gsap.to(link, { duration: 0.2, color: "#242424", ease: "power1.inOut"}); // Using color from existing hover state
      });
    ...
    socialIcons.forEach(icon => {
        icon.addEventListener("mouseenter", () => {
            gsap.to(icon, { duration: 0.2, y: -3, color: "#242424", ease: "power1.out" });
        });
        icon.addEventListener("mouseleave", () => {
            gsap.to(icon, { duration: 0.2, y: 0, color: "#fff", ease: "power1.inOut" });
        });
    });
